...
export async function NavBar(p: Partial<z.input<typeof navBarSchema>> & { locale: string }) {
  const parsed = navBarSchema.safeParse(p);
  const data = parsed.success ? parsed.data : navBarSchema.parse({});
  return <NavBarClient {...data} locale={p.locale} />;
}
