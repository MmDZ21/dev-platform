...
export default function MonthlySalesChart() {
  const { data } = trpc.adminStats.getMonthlyCounts.useQuery({
    modelKey: "Product",
    months: 12,
    dateField: "createdAt",
  });

  const categories = useMemo(() => data?.points.map((p) => p.label) ?? [], [data]);
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white px-5 pt-5 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 sm:pt-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Monthly Products</h3>
